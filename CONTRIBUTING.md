...
| `scripts/Install-Superpowers.Common.psm1` | 公共安装函数 |
| `install-*.ps1` | 各宿主安装脚本 |
| `install-all.ps1` | 批量安装入口 |
| `update-all.ps1` | 更新当前仓库并重装 |
| `refresh-upstream-and-reinstall.ps1` | 刷新 vendored upstream 后强制重装 |
...
```powershell
pwsh .\install-all.ps1 -Targets All -Scope Project -ProjectRoot E:\path\to\sandbox -Force
```
...
```powershell
pwsh .\refresh-upstream-and-reinstall.ps1 -Targets All -Scope Project -ProjectRoot E:\path\to\sandbox
```

如果你已经有一个本地 upstream checkout：

```powershell
pwsh .\refresh-upstream-and-reinstall.ps1 -SourcePath E:\path\to\superpowers -Targets All -Scope Project -ProjectRoot E:\path\to\sandbox
```
