...
- 如果上下文里已经有计划，直接说“按这份计划继续实现”，最好顺手点明是哪一份；不用要求固定目录，关键是别让它重新选范围。
...
```text
方案已经定了，你直接把它拆成可执行的实施计划。保存位置如果我指定了就按我指定的来；没指定时按仓库现有文档习惯放，并明确告诉我你写到了哪一份文件。步骤要具体到文件和验证方式，中文输出。
```
...
```text
按刚写好的那份计划继续实现，不要重新从 TODO 或别的清单改选范围。需要并行时只让 subagent 做调研和比选，代码改动由主线程完成。
```
