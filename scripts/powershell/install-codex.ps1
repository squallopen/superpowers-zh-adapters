...
$overlayRoot = Join-Path $repoRoot "templates/codex/skill-overlays"
...
$agentsBlock = Expand-TemplateFile `
    -TemplatePath (Join-Path $repoRoot "templates/codex/AGENTS.block.md") `
    -Tokens @{
...
Backup-ExistingFile -Path $agentsPath -Reason "更新 Codex 的 AGENTS.md 中 superpowers 说明段前先备份。" -BackupRoot $fileBackupRoot | Out-Null

Upsert-ManagedBlock `
    -Path $agentsPath `
    -BlockId "superpowers-codex" `
    -Content $agentsBlock
